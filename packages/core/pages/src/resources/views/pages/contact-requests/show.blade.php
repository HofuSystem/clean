    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div class="form-group mb-3 col-md-12">
                ...
                @isset($item->service)
                    <div class="alert alert-primary m-1" role="alert"><a href="{{ route('dashboard.testimonials.show',$item->service->id) }}">{{ $item?->service?->name ?? 'N/A' }}</a></div>
                @endisset
            ...
            <div class="form-group mb-3 col-md-12">
                <label>{{ trans("notes") }}</label><p>{{ $item->notes ?? 'N/A' }}</p>
            </div>
        
                </div>
                <div class="card-footer">
                    <a href="{{ route('dashboard.contact-requests.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> {{ trans('Back') }}</a>
                </div>
            </div>
                    ...
                    @include('comment::inc.comment-section', [
                        'commentUrl' => route('dashboard.contact-requests.comment', $item->id),
                    ])
                </div>
                <!--end::Card-->
            </div>
