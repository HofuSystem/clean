...
@section('content')
    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div class="form-group mb-3 col-md-12">
                <label>{{ trans("image") }}</label><div class="input-gallery "><div class="img-row " >{!!  Core\MediaCenter\Helpers\MediaCenterHelper::getImagesHtml($item->image) !!}</div></div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    <thead class="table-dark">
                        <tr>
                            <th>{{ trans('title (en)') }}</th><th>{{ trans('title (ar)') }}</th><th>{{ trans('small title (en)') }}</th><th>{{ trans('small title (ar)') }}</th><th>{{ trans('description (en)') }}</th><th>{{ trans('description (ar)') }}</th><th>{{ trans('images') }}</th><th>{{ trans('template') }}</th><th>{{ trans('actions') }}</th>
                        </tr>
                    </thead>
                    <tbody>
        ...
        @foreach ($item->sections ?? [] as $sItem)
            <tr>
    <td>{{ $sItem?->translate('en')?->title ?? 'N/A' }}</td><td>{{ $sItem?->translate('ar')?->title ?? 'N/A' }}</td><td>{{ $sItem?->translate('en')?->small_title ?? 'N/A' }}</td><td>{{ $sItem?->translate('ar')?->small_title ?? 'N/A' }}</td><td>{{ $sItem?->translate('en')?->description ?? 'N/A' }}</td><td>{{ $sItem?->translate('ar')?->description ?? 'N/A' }}</td><td>{!!  Core\MediaCenter\Helpers\MediaCenterHelper::getImagesHtml($item->images) !!} </td><td>{{ $sItem->template ?? 'N/A' }}</td>
        ...
        @endforeach
                ...
                <div class="card-footer">
                    <a href="{{ route('dashboard.pages.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> {{ trans('Back') }}</a>
                </div>
            </div>
                    ...
                    @include('comment::inc.comment-section', [
                        'commentUrl' => route('dashboard.pages.comment', $item->id),
                    ])
                </div>
                <!--end::Card-->
            </div>
