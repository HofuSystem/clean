...
@section('content')
    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div class="form-group mb-3 col-md-12">
                <label>{{ trans("publish date") }}</label><p>{{ $item->publish_date ?? 'N/A' }}</p>
            </div>
        
            <div class="form-group mb-3 col-md-12">
                <label>{{ trans("expired date") }}</label><p>{{ $item->expired_date ?? 'N/A' }}</p>
            </div>
        
            <div class="form-group mb-3 col-md-12">
                <label>{{ trans("next vision hour") }}</label><p>{{ $item->next_vision_hour ?? 'N/A' }}</p>
            </div>
            ...
            <div class="form-group mb-3 col-md-12">
                ...
                <div class="card-footer">
                    <a href="{{ route('dashboard.banner-notifications.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> {{ trans('Back') }}</a>
                </div>
            </div>
        

                    @include('comment::inc.comment-section',['commentUrl'=>route('dashboard.banner-notifications.comment',$item->id)])
                </div>
                <!--end::Card-->
            </div>
            <!--end::Container-->
