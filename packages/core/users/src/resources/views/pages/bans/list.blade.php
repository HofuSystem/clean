            ...
            <div class="card">
                        ...
                        <div  data-kt-user-table-toolbar="base">
                            <div class="d-flex justify-content-between">
                                <div class="">
                                     <div class="d-flex">
                                        ...
                                        <div class="border border-dashed border-success text-success rounded mx-1 p-2">
                                            <a href="{{ route('dashboard.bans.index') }}">
                                        ...
                                        <div class="border border-dashed border-danger  text-danger rounded mx-1 p-2">
                                            <a href="{{ route('dashboard.bans.index',['trash' => 1]) }}">
                                ...
                                <div class="d-flex flex-wrap">
                                    ...
                                    @can('dashboard.bans.export')
                                        <a href="{{ route('dashboard.bans.export') }}" id="export" type="button"
                                            class="btn-operation ">
                                    ...
                                    @endcan
                                    @can('dashboard.bans.import')
                                        <a href="{{ route('dashboard.bans.import') }}"
                                            class="btn-operation">
                                    ...
                                    @endcan
                                    <!--begin::Add -->
                                    @can('dashboard.bans.create')
                                        <a href="{{ route('dashboard.bans.create') }}"
                                            class="btn-operation ">
                            ...
                            <div id="collapseOne" class="accordion-collapse collapse" aria-labelledby="headingOne"
                                ...
                                <div class="p-1 row" data-kt-user-table-filter="form">
                                        ...
                                        <div class="col-md-6 mb-1">
                                            ...
                                            <select class="custom-select filter-input form-select advance-select" name="level" id="level">
                                                <option value=""> @lang("select level")</option><option value="ip" @selected("ip" == request("level")) >{{trans("ip")}}</option><option value="device" @selected("device" == request("level")) >{{trans("device")}}</option><option value="user" @selected("user" == request("level")) >{{trans("user")}}</option>
                                            </select>
                                        </div>
                                    
                                        <div class="col-md-6 mb-1">
                                            <label for="value"> @lang("value") </label>
                                            <input type="text" name="value" class="form-control filter-input"
                                                placeholder="@lang("search for value") " value="{{ request("value") }}">
                                        </div>
                                        <div class="col-md-6 mb-1">
                                            ...
                                            <select class="custom-select filter-input form-select advance-select" name="admin_id" id="admin_id">
                                                ...
                                                <option  value="" > @lang("select admins")</option>
                                                @foreach($admins as $item)
                                                    <option value="{{$item->id }}" @selected($item->id  == request("admin_id")) >@lang($item->fullname)</option>
                                                @endforeach
                                        ...
                                        <input type="number" name="reason" class="form-control filter-input"
                                            placeholder="@lang("search for reason") " value="{{ request("reason") }}">
                                        </div>
                                        <div class="col-md-6 mb-1">
                                        <label for="from"> @lang("starts from") </label>
                                        <input type="datetime-local" name="from_from" class="form-control filter-input"
                                            placeholder="@lang("search for starts") " value="{{ request("from") }}">
                                            </div>
                                            <div class="col-md-6 mb-1">
                                            <label for="from"> @lang("starts to") </label>
                                            <input type="datetime-local" name="to_from" class="form-control filter-input"
                                                placeholder="@lang("search for starts") " value="{{ request("from") }}">
                                        </div>
                                        <div class="col-md-6 mb-1">
                                        <label for="to"> @lang("ends from") </label>
                                        <input type="datetime-local" name="from_to" class="form-control filter-input"
                                            placeholder="@lang("search for ends") " value="{{ request("to") }}">
                                            </div>
                                            <div class="col-md-6 mb-1">
                                            <label for="to"> @lang("ends to") </label>
                                            <input type="datetime-local" name="to_to" class="form-control filter-input"
                                                placeholder="@lang("search for ends") " value="{{ request("to") }}">
                                        </div>
                                        <div class="col-md-6 mb-1">
                ...
                <div class="card-body pt-0 table-responsive table-responsive">
                    ...
                    <table class="table align-middle text-center table-row-dashed fs-6 gy-5" id="view-datatable"
                        data-load="{{ route('dashboard.bans.index',['trash' => request()->trash]) }}">
                        <!--begin::Table head-->
                        <thead class="table-primary">
                            ...
                            <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                                    ...
                                    <th class="text-center p-0" data-name="level">@lang("level")</th>
                                    <th class="text-center p-0" data-name="value">@lang("value")</th>
                                    <th class="text-center p-0" data-name="admin_id">@lang("admin")</th>
                                    <th class="text-center p-0" data-name="reason">@lang("reason")</th>
                                    <th class="text-center p-0" data-name="from">@lang("starts")</th>
                                    <th class="text-center p-0" data-name="to">@lang("ends")</th>
...
@push('js')
<script>
    var deleteUrl = "{{ route('dashboard.bans.delete', ['id'=>'%s','trash'=>request()->trash]) }}"
</script>
@endpush
