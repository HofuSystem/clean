        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div id="kt_content_container" class="container-fluid">
                ...
                <div class="card">
                    ...
                    <form class="form" method="POST" id="operation-form"  redirect-to="{{route("dashboard.favs.index")}}" data-id="{{$item->id ?? null}}"
                        @isset($item)
                            action="{{ route("dashboard.favs.edit",$item->id) }}"
                            data-mode="edit"
                        @else
                            action="{{ route("dashboard.favs.create") }}"
                            data-mode="new"
                        @endisset
                        >
                        ...
                        <div class="card-body row">
                            ...
                            <div class="form-group mb-3 col-md-6">
                                <label class="required" for="favs_type">{{ trans("favs type") }}</label>
                                <input type="text" name="favs_type" class="form-control "
                                    placeholder="{{ trans("Enter favs type") }} " value="@isset($item){{ $item->favs_type }}@endisset">
                                    
                            </div>

                            <div class="form-group mb-3 col-md-6">
                                <label class="required" for="favs_id">{{ trans("favs") }}</label>
                                <input type="text" name="favs_id" class="form-control "
                                    placeholder="{{ trans("Enter favs") }} " value="@isset($item){{ $item->favs_id }}@endisset">
