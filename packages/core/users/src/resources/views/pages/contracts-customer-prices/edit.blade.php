            ...
            <div id="kt_content_container" class="container-fluid">
                ...
                <div class="card">
                    ...
                    <form class="form" method="POST" id="operation-form" data-id="{{$item->id ?? null}}"
                        @if(isset($item))
                            action="{{ route("dashboard.contracts-customer-prices.edit",$item->id) }}"
                            data-mode="edit"
                        @else
                            action="{{ route("dashboard.contracts-customer-prices.create") }}"
                            data-mode="new"
                        @endif
                        >
                        ...
                        <div class="card-body row">
                            ...
                            <div class="form-group mb-3 col-md-12">
                                ...
                                <select class="custom-select  form-select advance-select" name="contract_id" id="contract_id"  >
                                    
                                    <option   value="" >{{trans("select")." ".trans("contract")}}</option>
                                    @foreach($contracts ?? [] as $sItem)
                                        <option data-id="{{$sItem->id }}" @selected(isset($item)  and $item->contract_id == $sItem->id) value="{{$sItem->id }}" >{{$sItem->title}}</option>
                                    @endforeach
                            ...
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <input type="number" name="over_price" class="form-control "
                                    placeholder="{{ trans("Enter over price") }} " value="{{ old("over_price" , $item->over_price ?? null) }}">
