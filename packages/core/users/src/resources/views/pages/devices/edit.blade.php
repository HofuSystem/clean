        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div id="kt_content_container" class="container-fluid">
                ...
                <div class="card">
                    ...
                    <form class="form" method="POST" id="operation-form"  redirect-to="{{route("dashboard.devices.index")}}" data-id="{{$item->id ?? null}}"
                        @isset($item)
                            action="{{ route("dashboard.devices.edit",$item->id) }}"
                            data-mode="edit"
                        @else
                            action="{{ route("dashboard.devices.create") }}"
                            data-mode="new"
                        @endisset
                        >
                        ...
                        <div class="card-body row">
                            ...
                            <div class="form-group mb-3 col-md-6">
                                <label class="required" for="device_token">{{ trans("device token") }}</label>
                                <input type="text" name="device_token" class="form-control "
                                    placeholder="{{ trans("Enter device token") }} " value="@isset($item){{ $item->device_token }}@endisset">
            ...
            <option value="ios" @selected(isset($item) and $item->type == "ios" ) >{{trans("ios")}}</option>
                                    <option value="android" @selected(isset($item) and $item->type == "android" ) >{{trans("android")}}</option>
                                    <option value="huawei" @selected(isset($item) and $item->type == "huawei" ) >{{trans("huawei")}}</option>
