...
class AuthController extends Controller
{
    ...
    public function login(ProviderLoginRequest $request)
    {
        if (!Auth::attempt($this->getCredentials($request))) {
            return $this->returnErrorMessage(trans('Invalid phone number or password'), [], [], 422);
        }
        $user = auth('api')->user();
        if (!$user->hasRole('driver'))
        return $this->returnErrorMessage(trans('you ara not driver'),[],[],422);
        ...
        // Generate token
        $user->token = $user->createToken('driver-login')->plainTextToken;
