...
@section('content')
    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div class="form-group mb-3 col-md-12">
                <label>{{ trans("phonecode") }}</label><p>{{ $item->phonecode ?? 'N/A' }}</p>
            </div>
        
            <div class="form-group mb-3 col-md-12">
                <label>{{ trans("short name") }}</label><p>{{ $item->short_name ?? 'N/A' }}</p>
            </div>
        
            <div class="form-group mb-3 col-md-12">
                <label>{{ trans("flag") }}</label><p>{{ $item->flag ?? 'N/A' }}</p>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    <thead class="table-primary">
                        <tr>
                            <th>{{ trans('name (en)') }}</th><th>{{ trans('name (ar)') }}</th><th>{{ trans('postal code') }}</th><th>{{ trans('image') }}</th><th>{{ trans('status') }}</th><th>{{ trans('actions') }}</th>
                        </tr>
                    </thead>
                    <tbody>
        ...
        @foreach ($item->cities ?? [] as $sItem)
            <tr>
    <td>{{ $sItem?->translate('en')?->name ?? 'N/A' }}</td><td>{{ $sItem?->translate('ar')?->name ?? 'N/A' }}</td><td>{{ $sItem->postal_code ?? 'N/A' }}</td><td>{!!  Core\MediaCenter\Helpers\MediaCenterHelper::getImagesHtml($item->image) !!} </td><td>{{ $sItem->status ?? 'N/A' }}</td>
        ...
        @endforeach
                ...
                <div class="card-footer">
                    <a href="{{ route('dashboard.countries.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> {{ trans('Back') }}</a>
                </div>
            </div>
        

                    @include('comment::inc.comment-section',['commentUrl'=>route('dashboard.countries.comment',$item->id)])
                </div>
                <!--end::Card-->
            </div>
            <!--end::Container-->
