...
@section('content')
    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div class="form-group mb-3 col-md-6">
                <label>{{ trans("delivery price") }}</label><p>{{ $item->delivery_price ?? 'N/A' }}</p>
            </div>
            ...
            <div class="form-group mb-3 col-md-12">
                ...
                @isset($item->country)
                    <div class="alert alert-primary m-1" role="alert"><a href="{{ route('dashboard.countries.show',$item->country->id) }}">{{ $item?->country?->name ?? 'N/A' }}</a></div>
                @endisset
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    <thead class="table-primary">
                        <tr>
                            <th>{{ trans('name (en)') }}</th><th>{{ trans('name (ar)') }}</th><th>{{ trans('lat') }}</th><th>{{ trans('lng') }}</th><th>{{ trans('postal code') }}</th><th>{{ trans('actions') }}</th>
                        </tr>
                    </thead>
                    <tbody>
        ...
        @foreach ($item->districts ?? [] as $sItem)
            <tr>
    <td>{{ $sItem?->translate('en')?->name ?? 'N/A' }}</td><td>{{ $sItem?->translate('ar')?->name ?? 'N/A' }}</td><td>{{ $sItem->lat ?? 'N/A' }}</td><td>{{ $sItem->lng ?? 'N/A' }}</td><td>{{ $sItem->postal_code ?? 'N/A' }}</td>
        ...
        @endforeach
                ...
                <div class="card-footer">
                    <a href="{{ route('dashboard.cities.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> {{ trans('Back') }}</a>
                </div>
            </div>
        

                    @include('comment::inc.comment-section',['commentUrl'=>route('dashboard.cities.comment',$item->id)])
                </div>
                <!--end::Card-->
            </div>
            <!--end::Container-->
