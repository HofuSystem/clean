        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div id="kt_content_container" class="container-fluid">
                ...
                <div class="card">
                    ...
                    <form class="form" method="POST" id="operation-form"  redirect-to="{{route("dashboard.cities.index")}}" data-id="{{$item->id ?? null}}"
                        @isset($item)
                            action="{{ route("dashboard.cities.edit",$item->id) }}"
                            data-mode="edit"
                        @else
                            action="{{ route("dashboard.cities.create") }}"
                            data-mode="new"
                        @endisset
                        >
                        ...
                        <div class="col-12 mt-5">
                            ...
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <input type="number" name="lat" class="form-control "
                                    placeholder="{{ trans("Enter lat") }} " value="{{ old("lat" , $item->lat ?? null) }}">
                            ...
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <input type="number" name="lng" class="form-control "
                                    placeholder="{{ trans("Enter lng") }} " value="{{ old("lng" , $item->lng ?? null) }}">
                            ...
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <input type="text" name="postal_code" class="form-control "
                                    placeholder="{{ trans("Enter postal code") }} " value="@isset($item){{ $item->postal_code }}@endisset">
                            ...
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <input type="number" name="delivery_price" class="form-control "
                                    placeholder="{{ trans("Enter delivery price") }} " value="{{ old("delivery_price" , $item->delivery_price ?? null) }}">
            ...
            <option value="active" @selected(isset($item) and $item->status == "active" ) >{{trans("active")}}</option>
                            ...
                            <div class="form-group mb-3 col-md-12">
                                <label class="required" for="country_id">{{ trans("country") }}</label>
                                <select class="custom-select  form-select advance-select" name="country_id" id="country_id"  >
                                    
                                    <option   value="" >{{trans("select country")}}</option>
                                    @foreach($countries ?? [] as $sItem)
                                        <option data-id="{{$sItem->id }}" @selected(isset($item)  and $item->country_id == $sItem->id) value="{{$sItem->id }}" >{{$sItem->name}}</option>
                                    @endforeach
                            ...
                            <div class="mt-3 items-container"
                                data-items-on       =   "city_id"
                                data-items-name     =   "districts"
                                data-items-from     =   "districts">

                                <h3 class="text-dark">{{ trans("districts") }}</h3>
                                ...
                                <div class="table-responsive ">
                                    <table class="table table-striped table-hover text-center" >
                                        <thead class="table-primary text-white text-capitalize h6">
                                            ...
                                            <th  scope="col" data-name="lat" data-type="number">{{ trans("lat") }}</th>
                                            <th  scope="col" data-name="lng" data-type="number">{{ trans("lng") }}</th>
                                        ...
                                        @foreach ($item->districts ?? [] as $sItem)
                ...
                <div class="modal fade" id="districtsModal" aria-hidden="true" aria-labelledby="districtsModalLabel" data-store="{{route("dashboard.districts.create")}}" >
                    <div class="modal-dialog modal-fullscreen">
                    <div class="modal-content">
                        <div class="modal-header">
                        <h1 class="modal-title fs-5" id="districtsModalLabel">{{ trans("District") }}</h1>
                ...
                <div class="modal fade" id="districtsDeleteModel" tabindex="-1" aria-labelledby="districtsDeleteModelLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="districtsDeleteModelLabel">{{ trans("Delete District") }} <span></span></h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                {{ trans("Are you sure you want to delete the District") }} <span></span>?
                            </div>
