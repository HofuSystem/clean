...
use Core\B2B\Requests\ContractsRequest; 
use Core\B2B\Requests\ImportContractsRequest; 
use Core\B2B\Exports\ContractsExport; 
use Core\B2B\Services\ContractsService;
...
class ContractsController extends Controller
{
    ...
    public function index(){
		...
		$companies = $this->companiesService->selectable(['id','fullname']);
		$contracts = $this->contractsService->selectable(['id','title']);
		$products = $this->productsService->selectable('id','name');

        return view('b2b::pages.contracts.list', compact('title','screen','clients','companies','contracts','products',"total","trash"));
    }
    ...
    public function createOrEdit(Request $request,$id = null){
        ...
        return view('b2b::pages.contracts.edit', compact('item','title','screen','clients','companies','contracts','products') );
    }
    ...
    public function show($id){
        ...
        return view('b2b::pages.contracts.show', compact('title','screen','item','comments'));
    }
    ...
    public function qrCodesForm(Request $request){
        ...
        return view('b2b::pages.contracts.qr-codes-form', compact('title','screen','contracts'));
    }
    ...
    public function generateQrCodes(Request $request){
        ...
        if ($request->has_titles == '1') {
            foreach ($request->titles as $title) {
                ...
                $qrCodes[] = [
                    ...
                    'title' => $title,
                    'owner' => ($contract->company?->fullname) ?? ($contract->client?->fullname ?? 'N/A')
                ];
            }
        } else {
        ...
        return view('b2b::pages.contracts.qr-codes-display', compact('title','screen','qrCodes'));
    }
