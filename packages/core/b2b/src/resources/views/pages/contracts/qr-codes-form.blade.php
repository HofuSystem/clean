                    ...
                    <div class="card-body row">
                        ...
                        <div class="form-group mb-3 col-md-12">
                            ...
                            <select class="custom-select form-select advance-select" name="contract_id" id="contract_id"
                                ...
                                <option @if($contract->id == request('contract_id')) selected @endif value="{{
                                    $contract->id }}"> {{ trans('contract') }} : {{ $contract->title }} - {{
                                    trans('client') }} : {{ $contract->client?->fullname }}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="form-group mb-3 col-md-12">
                            <label class="required" for="has_titles">{{ trans("Does every QR code have its own title?")
                                }}</label>
                        ...
                        <div class="form-group mb-3 col-md-12" id="count_field">
                            ...
                            <input type="number" name="count" id="count" class="form-control"
                                placeholder="{{ trans('Enter number of QR codes') }}" min="1" max="100" value="1"
                                required>
                        </div>
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.remove-title', function () {
            if ($('.title-input-group').length > 1) {
                ...
                Swal.fire({
                    text: '{{ trans("At least one title is required") }}',
                    icon: "error",
        ...
        $('form').on('submit', function (e) {
            ...
            if (hasTitles == '0') {
                if (!$('#count').val() || $('#count').val() < 1) {
                    e.preventDefault();
                    Swal.fire({
                        text: '{{ trans("Please enter a valid number of QR codes") }}',
                        icon: "error",
                        buttonsStyling: false,
                        confirmButtonText: "@lang('Ok, got it!')",
                        customClass: {
                ...
                if (titles.length === 0) {
                    e.preventDefault();
                    Swal.fire({
                        text: '{{ trans("Please enter at least one title") }}',
                        icon: "error",
                        buttonsStyling: false,
                        confirmButtonText: "@lang('Ok, got it!')",
                        customClass: {
