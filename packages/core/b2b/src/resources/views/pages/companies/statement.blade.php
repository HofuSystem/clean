        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack flex-wrap gap-2">
            ...
            <div class="page-title d-flex align-items-center flex-wrap me-3 mb-2">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                    <li class="breadcrumb-item text-muted"><a href="" class="text-muted text-hover-primary">@lang('Home')</a></li>
                    <li class="breadcrumb-item"><span class="bullet bg-gray-200 w-5px h-2px"></span></li>
                    <li class="breadcrumb-item text-muted"><a href="{{ route('dashboard.companies.index') }}" class="text-muted text-hover-primary">@lang('companies')</a></li>
                    <li class="breadcrumb-item"><span class="bullet bg-gray-200 w-5px h-2px"></span></li>
                ...
                <div class="me-3">
                    <select class="form-select form-select-sm form-select-solid" onchange="window.location.href=this.value">
                        @foreach($companies as $comp)
                            <option value="{{ route('dashboard.company-statement.show', $comp->id) }}" {{ $comp->id == $company->id ? 'selected' : '' }}>
                                {{ $comp->fullname }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div id="kt_content_container" class="container-fluid">
            ...
            <div id="action-panel" class="card mb-5 d-none">
                <div class="card-body position-relative">
                    <button type="button" class="btn-close position-absolute top-0 end-0 m-3" onclick="closeForm()"></button>
                    ...
                    <div id="owed-form-wrapper"  class="d-none">
                        ...
                        <form id="owed-form" class="row g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" name="reference_id" class="form-control bg-ranger border-danger" value="{{ $nextOwedRefrence }}" readonly>
                            </div>
                            <div class="col-md-2">
            ...
            <div id="statement-doc" class="card shadow-sm" style="max-width:900px;margin:0 auto 60px;">
                ...
                <div class="statement-paper p-5" style="font-family:'Tajawal','Segoe UI',sans-serif; direction:rtl;">

                    {{-- Header --}}
                    <div class="d-flex justify-content-between align-items-start mb-4 pb-3 border-bottom">
                        <div style="text-align:right;">
                            <h2 class="fw-bold mb-0" style="font-size:1.5rem;">@lang('Customer Account Statement')</h2>
                            <p class="text-muted mb-0" style="letter-spacing:3px;font-size:.7rem;text-transform:uppercase;">STATEMENT OF ACCOUNT</p>
                            <p class="mb-0 mt-2 fw-semibold">{{ $sellerNameAr }}</p>
                        ...
                        @if($sellerLogo)
                            <img src="{{ asset($sellerLogo) }}" alt="{{ $sellerName }}" style="height:70px;object-fit:contain;">
                        @else
                            <div style="width:80px;height:80px;background:#00AEEF;border-radius:8px;display:flex;align-items:center;justify-content:center;">
                    ...
                    <div class="d-flex justify-content-between align-items-start mb-4 gap-4">
                        ...
                        {{-- Billed to box --}}
                        <div class="border rounded p-3 text-end" style="min-width:260px;font-size:.82rem;line-height:1.9;background:#f9f9f9;">
                            <p class="text-muted mb-1" style="font-size:.65rem;letter-spacing:2px;text-transform:uppercase;">@lang('BILLED TO / Customer')</p>
                            <p class="fw-bold mb-1" style="font-size:1rem;">{{ $company->fullname }}</p>
                                ...
                                @forelse($rows as $row)
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            ...
                                            @if($row['attachment'])
                                                <a href="{{ asset('storage/' . $row['attachment']) }}" target="_blank" class="ms-2 text-primary" title="@lang('View Attachment')">
                                            ...
                                            @elseif($row['type'] === 'owed')
                                                <a href="{{ route('dashboard.company-statement.print-credit-note', ['companyId' => $company->id, 'financialId' => $row['id']]) }}" 
                                                   target="_blank" class="ms-2 text-dark" title="@lang('Print Credit Note')">
                                                    إشعار دائن ضريبي
                                                    <i class="fas fa-print"></i>
                                                </a>
                                            @endif
                                        ...
                                        <td class="{{ $row['balance'] > 0 ? 'text-danger' : ($row['balance'] < 0 ? 'text-success' : 'text-muted') }} fw-bold">
                                            {{ number_format(abs($row['balance']), 2) }}
                                            <br>
                                            <small style="font-size:.65rem;">
                                                @if($row['balance'] > 0) @lang('(Debit)') @elseif($row['balance'] < 0) @lang('(Credit)') @else — @endif
                                            </small>
                                        </td>
                                    </tr>
                                @empty
                    ...
                    <div class="d-flex justify-content-between align-items-start mt-3">
                        <div class="border rounded p-3 text-center" style="min-width:200px;">
                            <p class="mb-1 fw-bold" style="font-size:1.4rem;" class="{{ $balance > 0 ? 'text-danger' : 'text-success' }}">
                                {{ number_format(abs($balance), 2) }}
                            </p>
                            @if($balance > 0)
                                <span class="badge bg-danger">@lang('Required from you')</span>
                            @elseif($balance < 0)
                                <span class="badge bg-success">@lang('Required for you')</span>
                            @else
                                <span class="badge bg-secondary">@lang('Settled')</span>
                            @endif
                            <p class="mt-1 mb-0 fw-semibold text-muted" style="font-size:.75rem;">@lang('Final Outstanding Balance')</p>
                        </div>
                    ...
                    <div class="text-center mt-5 pt-3 border-top" style="font-size:.72rem;color:#999;">
                        <p class="mb-0">@lang('An approved document automatically extracted from the financial system.')</p>
...
<style>
    /* ── Panel animation ── */
    #action-panel { transition: all .3s ease; border-left: 4px solid transparent; }
    #action-panel.owed { border-left-color: #f1416c !important; }
    #action-panel.paid { border-left-color: #50cd89 !important; }

    /* ── Statement paper look ── */
    .statement-paper { background: #fff; }

    /* ── Print styles ── */
    @media print {
        #kt_toolbar, #action-panel, .btn, nav, aside,
        #kt_aside, #kt_header, #kt_footer { display: none !important; }
        #statement-doc { box-shadow: none !important; max-width: 100% !important; margin: 0 !important; }
        .statement-paper { padding: 10mm !important; }
