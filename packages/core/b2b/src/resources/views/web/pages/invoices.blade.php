...
<div id="view-invoices" class="view-section active space-y-6">
    <div
        ...
        <div class="w-full md:w-64">
            <select id="invoice-month-select" class="select-premium w-full p-4 bg-white border border-gray-200 rounded-xl outline-none focus:border-[#1c75bc] transition-all font-bold text-gray-800">
                @forelse($monthlyInvoices as $invoice)
                    @php
                        $monthName = \Carbon\Carbon::createFromDate($invoice->year, $invoice->month, 1)->locale(app()->getLocale())->translatedFormat('F');
                    @endphp
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" id="invoice-stats">
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden dir-dependent-text">
            <div class="absolute top-0 right-0 w-1 h-full bg-blue-400"></div>
            <p class="text-gray-400 font-bold text-[10px] uppercase tracking-widest mb-2">{{ trans('client.contract_orders_cost') }}</p>
            <p class="text-2xl font-black" id="stat-contract">0.00</p>
        </div>
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden dir-dependent-text">
            <div class="absolute top-0 right-0 w-1 h-full bg-purple-400"></div>
            <p class="text-gray-400 font-bold text-[10px] uppercase tracking-widest mb-2">{{ trans('client.total_guest_orders') }}</p>
            <p class="text-2xl font-black" id="stat-guest">0.00</p>
        </div>
        <div class="bg-green-50 p-6 rounded-2xl border border-green-100/50 shadow-sm relative overflow-hidden dir-dependent-text">
            <div class="absolute top-0 right-0 w-1 h-full bg-green-500"></div>
            <p class="text-green-700 font-bold text-[10px] uppercase tracking-widest mb-2">{{ trans('client.hotel_profit_guests') }}</p>
        ...
        <div class="bg-gray-900 p-6 rounded-2xl shadow-lg relative overflow-hidden dir-dependent-text">
            <p class="text-gray-400 font-bold text-[10px] uppercase tracking-widest mb-2">{{ trans('client.due_invoice') }}</p>
            <p class="text-2xl font-black text-white"><span id="stat-total">0.00</span> <span class="text-sm text-gray-500">{{ trans('client.sar') }}</span></p>
        </div>
    </div>
    ...
    <div
        ...
        <div class="text-white text-center md:text-right z-10 dir-dependent-text">
            <h3 class="font-black text-2xl mb-2 tracking-tight">{{ trans('client.pay_invoice_for') }} <span id="pay-month-name"></span></h3>
