...
class CategoriesResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        $deliveryPrice = DeliveryPrice::where(function ($query) use ($categoryId) {
            $query->where('category_id', $categoryId)
                ->orWhereNull('category_id');
        })
            ->where(function ($query) use ($profile) {
                $query->where('city_id', $profile?->city_id)
                    ->orWhereNull('city_id');
            })
            ->where(function ($query) use ($profile) {
                $query->where('district_id', $profile?->district_id)
        ...
        return [
            ...
            "delivery_price" => DashboardDataTableFormatter::text($deliveryPrice?->price ?? $this->delivery_price),
