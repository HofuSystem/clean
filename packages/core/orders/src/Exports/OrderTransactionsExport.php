...
class OrderTransactionsExport implements FromCollection, WithHeadings, WithMapping, WithCustomCsvSettings
{
    ...
    public function headings(): array
    {
        ...
        if(empty($this->cols) or in_array('online_payment_method',$this->cols)){
            $headings[] = trans('online payment method');
        }
    ...
    public function map($model): array
    {
        ...
        if(empty($this->cols) or in_array('online_payment_method',$this->cols)){
            $data[] = $model->online_payment_method;
        }
        if(empty($this->cols) or in_array('amount',$this->cols)){
            $data[] = $model->amount;
        }
