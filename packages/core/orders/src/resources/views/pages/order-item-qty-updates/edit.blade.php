        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div id="kt_content_container" class="container-fluid">
                ...
                <div class="card">
                    ...
                    <form class="form" method="POST" id="operation-form"  redirect-to="{{route("dashboard.order-item-qty-updates.index")}}" data-id="{{$item->id ?? null}}"
                        @isset($item)
                            action="{{ route("dashboard.order-item-qty-updates.edit",$item->id) }}"
                            data-mode="edit"
                        @else
                            action="{{ route("dashboard.order-item-qty-updates.create") }}"
                            data-mode="new"
                        @endisset
                        >
                        ...
                        <div class="card-body row">
                            ...
                            <div class="form-group mb-3 col-md-12">
                                ...
                                <select class="custom-select  form-select advance-select" name="item_id" id="item_id"  >
                                    ...
                                    @foreach($items ?? [] as $sItem)
                                        <option data-id="{{$sItem->id }}" @selected(isset($item)  and $item->item_id == $sItem->id) value="{{$sItem->id }}" >{{$sItem->product_id}}</option>
                                    @endforeach
                            ...
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <input type="number" name="from" class="form-control "
                                    placeholder="{{ trans("Enter from") }} " value="{{ old("from" , $item->from ?? null) }}">
                            ...
                            <div class="form-group mb-3 col-md-6">
                                ...
                                <input type="number" name="to" class="form-control "
                                    placeholder="{{ trans("Enter to") }} " value="{{ old("to" , $item->to ?? null) }}">
                            ...
                            <div class="form-group mb-3 col-md-12">
                                ...
                                <input type="email" name="updater_email" class="form-control "
                                    placeholder="{{ trans("Enter updater email") }} " value="{{ old("updater_email" , $item->updater_email ?? null) }}">
