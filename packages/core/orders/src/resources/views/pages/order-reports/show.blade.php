...
@section('content')
    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div class="form-group mb-3 col-md-4">
                <label>{{ trans("report reason") }}</label>
                @isset($item->reportReason)
                    <div class="alert alert-primary m-1" role="alert"><a href="{{ route('dashboard.report-reasons.show',$item->reportReason->id) }}">{{ $item?->reportReason?->name ?? 'N/A' }}</a></div>
                @endisset
                
            </div>
        
            <div class="form-group mb-3 col-md-12">
                <label>{{ trans("desc location") }}</label><p>{{ $item->desc_location ?? 'N/A' }}</p>
            </div>
        
                </div>
                <div class="card-footer">
                    <a href="{{ route('dashboard.order-reports.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> {{ trans('Back') }}</a>
                </div>
            </div>
        

                    @include('comment::inc.comment-section',['commentUrl'=>route('dashboard.order-reports.comment',$item->id)])
                </div>
                <!--end::Card-->
            </div>
            <!--end::Container-->
