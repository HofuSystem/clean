        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            ...
            <div id="kt_content_container" class="container-fluid">
                ...
                <div class="card">
                    ...
                    <form class="form" method="POST" id="operation-form"  redirect-to="{{route("dashboard.order-types-of-datas.index")}}" data-id="{{$item->id ?? null}}"
                        @isset($item)
                            action="{{ route("dashboard.order-types-of-datas.edit",$item->id) }}"
                            data-mode="edit"
                        @else
                            action="{{ route("dashboard.order-types-of-datas.create") }}"
                            data-mode="new"
                        @endisset
                        >
                        ...
                        <div class="card-body row">
                            ...
                            <div class="form-group mb-3 col-md-12">
                                <label class="required" for="key">{{ trans("key") }}</label>
                                <input type="text" name="key" class="form-control "
                                    placeholder="{{ trans("Enter key") }} " value="@isset($item){{ $item->key }}@endisset">
                                    
                            </div>

                            <div class="form-group mb-3 col-md-12">
                                <label class="" for="value">{{ trans("value") }}</label>
                <textarea type="number" name="value" class="form-control "
                    placeholder="{{ trans("Enter value") }} " >@isset($item){{ $item->value }}@endisset</textarea>
