...
<footer id="footer" class="bg-gray-900 text-white pt-20 pb-10 border-t border-gray-800 mt-0">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16 text-center {{ $isRtl ? 'md:text-right' : 'md:text-left' }}">
            <div class="space-y-6 flex flex-col items-center {{ $isRtl ? 'md:items-start' : 'md:items-start' }}">
                ...
                <div class="flex gap-4 justify-center md:justify-start">
                    @if(setting('twitter'))<a href="{{ setting('twitter') }}" target="_blank" rel="noopener" class="text-gray-400 hover:text-brand-400 transition-colors" aria-label="Twitter"><i class="fa-brands fa-x-twitter text-xl"></i></a>@endif
                    @if(setting('instagram'))<a href="{{ setting('instagram') }}" target="_blank" rel="noopener" class="text-gray-400 hover:text-brand-400 transition-colors" aria-label="Instagram"><i class="fa-brands fa-instagram text-xl"></i></a>@endif
                    @if(setting('linkedin'))<a href="{{ setting('linkedin') }}" target="_blank" rel="noopener" class="text-gray-400 hover:text-brand-400 transition-colors" aria-label="LinkedIn"><i class="fa-brands fa-linkedin text-xl"></i></a>@endif
                    @if(setting('tiktok'))<a href="{{ setting('tiktok') }}" target="_blank" rel="noopener" class="text-gray-400 hover:text-brand-400 transition-colors" aria-label="TikTok"><i class="fa-brands fa-tiktok text-xl"></i></a>@endif
                    @if(setting('youtube'))<a href="{{ setting('youtube') }}" target="_blank" rel="noopener" class="text-gray-400 hover:text-brand-400 transition-colors" aria-label="YouTube"><i class="fa-brands fa-youtube text-xl"></i></a>@endif
                    @if(setting('facebook'))<a href="{{ setting('facebook') }}" target="_blank" rel="noopener" class="text-gray-400 hover:text-brand-400 transition-colors" aria-label="Facebook"><i class="fa-brands fa-facebook text-xl"></i></a>@endif
                </div>
            </div>
            ...
            <div class="flex flex-col items-center md:items-start">
                ...
                <ul class="space-y-3 text-sm text-gray-400">
                    ...
                    <li><a href="{{ route('pricing') }}" class="hover:text-brand-400 transition-colors">{{ $isRtl ? 'قائمة الأسعار' : 'Pricing List' }}</a></li>
                    <li><a href="{{ route('coverage') }}" class="hover:text-brand-400 transition-colors">{{ $isRtl ? 'تغطية المدن والأحياء' : 'Cities & Districts Coverage' }}</a></li>
                    <li><a href="{{ route('b2b') }}" class="hover:text-brand-400 transition-colors">{{ $isRtl ? 'خدمات الأعمال والشركات' : 'Corporate & B2B Solutions' }}</a></li>
                    <li><a href="{{ route('why-us') }}" class="hover:text-brand-400 transition-colors">{{ $isRtl ? 'لماذا كلين ستيشن' : 'Why Clean Station' }}</a></li>
            ...
            <div class="flex flex-col items-center md:items-start">
                ...
                <form action="{{ route('newsletter') }}" method="POST" class="relative mb-6 w-full max-w-xs mx-auto md:mx-0">
                    ...
                    <button type="submit" aria-label="Subscribe" class="absolute top-1/2 transform -translate-y-1/2 {{ $isRtl ? 'left-2' : 'right-2' }} text-brand-400 hover:text-white transition-colors"><i class="fa-solid fa-paper-plane"></i></button>
                </form>
